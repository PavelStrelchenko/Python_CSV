...
def get_data():  # функция ввода данных с консоли
    flag = False  # флаг, на который будет ориентироваться цикл
    while not flag:  # цикл работает, пока не получит значение False, по флагу
        try:  # проверка валидности данных при вводе
            ...
            flag = True  # меняем значение flag на True, чтобы завершить цикл
    return [first_name, last_name, phone]  # по итогу работы функции возвращаем список с данными


def create_file(filename):  # функция создания файла csv
    with open(filename, 'w', encoding='utf-8') as data:  # открываем файл для записи (w) и указываем его кодировку(для кириллицы)
        f_w = DictWriter(data, fieldnames=['Имя', 'Фамилия', 'Телефон'])  # добавляем объект DictWriter
...
def read_file(filename):  # функция чтения файла
    with open(filename, 'r', encoding='utf-8') as data:  # открываем файл для чтения (r) и указываем его кодировку
        f_r = DictReader(data)  # создаем объект DictReader
        return list(f_r)  # возвращаем список, состоящий из словарей


def write_file(filename, lst):  # функция записи в файл, filename - имя файла, lst - список, возвращаемый get_data()
    res = read_file(filename)  # кладём в переменную список словарей,возвращаемый функцией read_file
...
def row_search(filename):  # поиск по фамилии в файле csv
    last_name = input("Введите фамилию: ")  # ввод фамилии для поиска
...
def copy_row(filename, filename_2):
    row_num = int(input("Введите номер копируемой строки: "))
    res = read_file(filename) # читаем файл функцией read_file(), получаем список словарей
    res_2 = read_file(filename_2) # читаем файл функцией read_file(), получаем список словарей
    res_2.append(res[row_num - 1]) # копируем строку по номеру из одного файла в другой
    standard_write(filename_2, res_2) # записываем изменения в файл при помощи вызова функции standard_write()
...
def main():  # основной модуль с которого начинает работу программа
    while True:  # бесконечный цикл, в котором ожидаем реакции с консоли
        ...
        print(' "d"-удаление из csv; "c"-изменить строку в csv, "p" - копировать строку из одного файла в другой')
        ...
        if command == "q":
            ...
            if not exists (filename_2):
                create_file(filename_2)
            copy_row(filename, filename_2)



filename = 'phone.csv'  # задаем имя файла csv в переменную
filename_2 = 'phone_2.csv' 
